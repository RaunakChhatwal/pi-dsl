...
-- Get size of a Storable type without needing a value
sizeOf :: forall a. F.Storable a => Int
sizeOf = F.sizeOf (undefined :: a)

-- Get alignment of a Storable type without needing a value
alignment :: forall a. F.Storable a => Int
alignment = F.alignment (undefined :: a)

-- Generate a name for the i-th field offset variable
offsetName :: Int -> TH.Name
offsetName i = TH.mkName ("offset" ++ show i)

-- Generate a name for the i-th field variable
fieldName :: Int -> TH.Name
fieldName i = TH.mkName ("field" ++ show i)

-- Generate let declarations for cumulative field offsets in a struct
...
-- Generate a case match for peeking a specific constructor from memory
...
-- Generate a case match for poking a specific constructor to memory
...
-- Create a method definition from name, args, and body
implMethod :: TH.Name -> [TH.Pat] -> TH.Exp -> TH.Dec
implMethod methodName args body = TH.FunD methodName [TH.Clause args (TH.NormalB body) []]

-- Generate a Storable instance for a sum type (tagged union representation)
...
-- Generate a Storable instance for a newtype wrapper
...
-- Extract constructor name and field types from a TH constructor
...
-- Typeclass for recursively freeing memory owned by FFI-allocated C representations
class F.Storable a => Free a where
  -- Free allocations owned by the value at this pointer (not the pointer itself)
  freeInPlace :: F.Ptr a -> IO ()

  -- Free nested allocations then free the pointer itself
  free :: F.Ptr a -> IO ()
  free ptr = when (ptr /= F.nullPtr) $ freeInPlace ptr >> F.free ptr

-- Generate a case match for freeing a specific constructor's fields
...
-- Generate a Free instance for a sum type
...
-- Generate a Free instance for a newtype wrapper
...
-- Generate a Free instance for a type by inspecting its TH info
...
-- Generate an FFI export that frees a pointer and its nested allocations
...
-- Generate an FFI export wrapper for a function with Storable argument and return types
