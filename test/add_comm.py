...
n = Var("n")
@decl(env)
def add_zero(n: Var[Nat]) -> Term[n + Nat.zero == n]:
    motive = lam(lambda n: n + Nat.zero == n)
...
m = Var("m")
@decl(env)
def add_succ(n: Var[Nat], m: Var[Nat]) -> Term[n + Nat.succ(m) == Nat.succ(n + m)]:
    motive = lam(lambda n: n + Nat.succ(m) == Nat.succ(n + m))
...
@decl(env)
def add_comm(n: Var[Nat], m: Var[Nat]) -> Term[n + m == m + n]:
    motive = lam(lambda m: n + m == m + n)
